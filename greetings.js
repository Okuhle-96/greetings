module.exports = function (pool) {
    ...
    async function countNames(name, language) {
        ...
        if (regex.test(nameToUpp) && language) {
            var checkname = await pool.query(`SELECT username from users WHERE username = $1`, [nameToUpp]);

            if (checkname.rowCount < 1) {

                await pool.query(`INSERT INTO users (username,counters) VALUES ($1,$2)`, [nameToUpp, 1])
            }

            else {
                await pool.query(`UPDATE users SET counters = counters + 1 WHERE username = $1`, [nameToUpp])
    ...
    async function nameCounter(names) {
        var nameCount = await pool.query('select counters from users WHERE username = $1', [names])
